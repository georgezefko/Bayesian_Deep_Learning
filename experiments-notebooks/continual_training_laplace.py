...
def main(trained_model_filepath,parameterize=False, misplacement=False,load=True,save=True,subset=False):
    ...
    prior_mean = torch.zeros_like(torch.nn.utils.parameters_to_vector(model.parameters()))
...
def optimize_marglik(model, train_loader, sub_laplace, log_prior_prec, hyper_optimizer):
    N = len(train_loader.dataset)
    prior_prec = torch.exp(log_prior_prec)
    hyper_la = deepcopy(sub_laplace)
    hyper_la.prior_mean = sub_laplace.mean
    ...
    sub_laplace.prior_precision = hyper_la.prior_precision.clone()

    return log_prior_prec, -hyper_la.log_marginal_likelihood().detach().cpu().item()

def prior_prec_to_tensor(prior_prec, model,prior_structure='scalar'):
...
def train(model,optimizer,train_loader,sub_laplace,start_epoch,device,criterion,epochs=2):  
    ...
    lr_hyp = 0.1
    lam = 1.0
    log_pp_init = np.log(prior_prec_init)
    log_prior_prec = prior_prec_to_tensor(log_pp_init, model)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for data,target in train_loader:
            ...
            mean = torch.nn.utils.parameters_to_vector(model.parameters())
            

            loss = criterion(output, target) -lam * sub_laplace.log_prob(mean) / len(train_loader.dataset)
            loss.backward()
            optimizer.step()
        ...
        log_prior_prec, marglik = optimize_marglik(
             model, train_loader, sub_laplace, log_prior_prec, hyper_optimizer)

        print(f'epoch {epoch+1} - train loss: {train_loss:.3f}, neg. log marglik: {marglik:.3f}')
